...
def save_masked_mesh(verts, faces, keep_mask, path):
    ...
    trimesh.Trimesh(vertices=new_verts, faces=new_faces_remapped).export(path)
...
class LocalConfig:
    def __init__(self, config_path):
        ...
        cp.read(config_path)
        sec_model = 'model parameters'
        self.conf['z_length'] = cp.getint(sec_model, 'z_length')
        self.conf['batch_norm'] = cp.getboolean(sec_model, 'batch_norm')
        self.conf['num_features_global'] = [int(x) for x in cp.get(sec_model, 'num_features_global').split(',')]
        self.conf['num_features_local'] = [int(x) for x in cp.get(sec_model, 'num_features_local').split(',')]
        self.conf['down_sampling_factors'] = [float(x) for x in cp.get(sec_model, 'down_sampling_factors').split(',')]
        self.conf['n_layers'] = len(self.conf['num_features_global']) - 1
        sec_io = 'I/O parameters'
        self.conf['template_file'] = os.path.join(project_root, cp.get(sec_io, 'template_file'))
...
def run_batch_final_submission():
    ...
    print(f"Info: Starting MCG-Face reconstruction pipeline (Device: {device})")
    ...
    print("Info: Building mesh graph structure...")
    edge_index = get_edges_from_faces(mask_faces_np, device)
    print("Info: Calculating symmetry indices...")
    ...
    files = sorted([f for f in os.listdir(input_dir) if f.endswith('.obj')])[:12]
    ...
    for idx, f_name in enumerate(files):
        ...
        with torch.no_grad():
            ...
            M_target_full[:, 0] = -M_target_full[:, 0] # Flip X
        ...
        with torch.no_grad():
            ...
            trimesh.Trimesh(vertices=M_final.cpu().numpy(), faces=mask_faces_np).export(os.path.join(sample_dir, "2_reconstructed.obj"))
            trimesh.Trimesh(vertices=M_gt_norm.cpu().numpy(), faces=mask_faces_np).export(os.path.join(sample_dir, "0_ground_truth.obj"))
            save_masked_mesh(M_gt_norm, torch.tensor(mask_faces_np, device=device), keep_mask, os.path.join(sample_dir, "1_input_hole.obj"))

    print("\nBatch reconstruction process completed successfully.")
